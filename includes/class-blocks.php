...
if ( class_exists( 'DropdownRedirect\Blocks' ) ) {
	return;
}
...
class Blocks {
	...
	public function __construct( $plugin_name, $version ) {
		...
		$this->editor_script_handle = 'ddr-block-editor-script';
		$this->editor_style_handle  = 'ddr-block-editor';
		$this->public_style_handle  = 'ddr-block-public';
	...
	public function register_hooks() {
		...
		add_action( 'init', [ $this, 'create_block_ddr' ] );
	 ...
	 */
	public function create_block_ddr() {
		...
		if ( ! file_exists( $script_asset_path ) ) {
			throw new \Error(
				'You need to run `npm start` or `npm run build` for the "dropdown-redirect/create_block_ddr_simple" block first. Could Not find the index.asset.php file: ' . $script_asset_path
			);
		}
		...
		if ( function_exists( 'wp_set_script_translations' ) ) {
			 ...
			 */
			wp_set_script_translations( 'ddr-block-editor-script', 'dropdown-redirect' );
		}
	...
	public function get_build_dir( $file_name_with_sub_dir ) {
		...
		return DROPDOWN_REDIRECT_DIR_PATH . 'build' . DIRECTORY_SEPARATOR . $file_name_with_sub_dir;
	...
	public function get_build_url( $file_name_with_sub_dir ) {
		...
		return DROPDOWN_REDIRECT_URL_PATH . 'build/' . $file_name_with_sub_dir;
